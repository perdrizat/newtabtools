...
function getString(name) {
	return api.i18n.getMessage(name);
}
...
getTab().then(async tab => {
	let isPinned = await api.runtime.sendMessage({name: 'Tiles.isPinned', url: tab.url});
...
document.getElementById('pin').onclick = function() {
	getTab().then(function(tab) {
		api.runtime.sendMessage({name: 'Tiles.pinTile', title: tab.title, url: tab.url});
		window.close();
	});
};

document.getElementById('capture').onclick = async function() {
	await api.runtime.sendMessage({name: 'Thumbnails.capture'});
	window.close();
};
