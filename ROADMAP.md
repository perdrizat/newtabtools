...
- **High-severity findings — now resolved** — §2.1 (stored XSS via zip restore) fixed 2026-05-07: URL scheme allow-list at restore time (`export.js`) and render time (`fx-newTab.js`). §2.2 (vendored 2013 `zip.js`) fixed 2026-05-07: replaced with modern `@zip.js/zip.js` v2.8.26 (devDependency, dist file vendored). Both fixes landed under the Phase 1 characterization safety net as planned.
