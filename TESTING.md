...
- **Firefox-first, Chrome-supported:** This extension's primary target is Firefox via `browser_specific_settings.gecko` in `manifest.json`, running on **Manifest V3** (`strict_min_version` **152.0** — the version Firefox first exposes `tabs.captureVisibleTab`/`captureTab` to MV3 extensions; empirically bisected, see [`CONTRIBUTING.md`](CONTRIBUTING.md) "Decisions of record"), with a non-persistent **event page** background (classic `background.scripts` array, no service worker — full DOM/`window`/canvas access, suspends after idle and respawns on events). Chrome is a fully supported second target built from the same source tree via a manifest overlay ([`CHROME.md`](CHROME.md) — `minimum_chrome_version` 148, a real service-worker background, JSON messaging with a JSON-safe wire codec for binary payloads — CHROME.md Decision 11; structured clone is canary-gated in branded Chrome and must never be load-bearing); its E2E and UAT tiers are described alongside the Firefox ones below, and a Chrome Web Store release is pending. New platform divergence must route through the `api` capability seam (`webextension/api.js` / `webextension/lib/platform.js`), not ad-hoc `chrome.*`/`browser.*` branches — see [`CONTRIBUTING.md`](CONTRIBUTING.md) "Decisions of record".
...
Prefer **behavioral tests** that load the module via a native `import` and exercise it through its public API — every file under `webextension/` (page files and `lib/` alike) is a real ES module with real `export`s, fully typed and split into feature modules by the page-modules and chrome-prep C3/C4 arcs (see git history, `CHANGELOG.md`, and `audit/` for the per-arc record): `newTab.js` plus `grid.js`/`cell.js`/`site.js`/`page.js` (formerly one monolith, `fx-newTab.js`, dissolved in C4c). `newTab.js` and `grid.js`/`page.js` still reach each other via a dynamic (not static) `import()` at test time — real markup must be mounted first so their top-level DOM-wiring doesn't throw, and a static import can't sequence that. The helper in [`tests/integration/_helpers.ts`](tests/integration/_helpers.ts) that reduces the boilerplate for `Site`:

- **`mountSite(linkData)`** (async) — constructs a `Site` instance with the full tile environment (mounts the shipped `newTab.html` body, then `ensureSiteEnv()`'s dynamic import of `site.js`/`newTab.js`). Returns `{ site, node, cleanup }`; callers must `await` it.

`loadModule` (a `vm.createContext` sandbox loader for script-mode files) is gone — retired once its last consumer migrated to a native import (page-modules P4); `vm.runInThisContext`/`vm.createContext` are no longer how any production file is loaded here.
...
| **Branded Google Chrome (stable)** | current stable | Chrome **E2E parity** target (Decision 12, [`CHROME.md`](CHROME.md) — the production binary users run; landing with D8) + the branded smoke lane. Automated via Puppeteer's **pipe transport + CDP `Extensions.loadUnpacked`** — the one route that works on branded ≥ 137 (`--load-extension` is ignored there, and Selenium's port transport can't reach the `Extensions` CDP domain). | `google-chrome --version` |
| **Chrome for Testing (CfT), >= 148** | auto-provisioned | Chrome **UAT** target + local smoke + E2E fallback when no branded binary is present. **Fidelity caveat (the 2026-07-17 canary-gate incident):** CfT is an *unbranded* build whose channel detection passes canary-gated feature flags that branded stable rejects — it is faithful for engine/API behavior but NOT for channel gates; anything channel-sensitive must be proven on branded (see "Browser binary strategy" below). Provisioned via `pnpm chrome:provision`. See [`tests/e2e-chrome/README.md`](tests/e2e-chrome/README.md). | `pnpm chrome:provision` |

> Release-channel Firefox now serves **both** the E2E and UAT tiers — MV3's `tabs.captureVisibleTab`/`captureTab` only exist from Firefox 152.0, and no ESR build that new exists yet in Mozilla's APT repo (see [`CONTRIBUTING.md`](CONTRIBUTING.md) "Decisions of record"). When a 152-based ESR reaches the APT repo, E2E can move back to it; `$FIREFOX_ESR_BIN` still works as a binary override in the meantime. The bottom two (geckodriver, Claude Code CLI) are **only** needed to run `pnpm test:uat` (pre-release tier); Chrome for Testing is **only** needed to run the Chrome tier (`pnpm chrome:smoke`, `pnpm test:e2e:chrome`, `pnpm test:uat:chrome`). The Unit/Integration tiers don't require any browser binary at all. CI installs release Firefox always and provisions Chrome for Testing for its parallel `chrome-smoke` job, but doesn't install the UAT-only tools. The packages `selenium-webdriver` and `@modelcontextprotocol/sdk` arrive via `pnpm install`. Setup details under "Installing the E2E & UAT (Firefox) and verify tooling" below.
...
- **Chrome:** a parallel `chrome-smoke` job provisions Chrome for Testing (`pnpm chrome:provision`) and runs the Chrome smoke checks (`pnpm chrome:smoke` + the Selenium variant). A **branded-stable smoke job** on the runner's *preinstalled* Google Chrome (no provisioning; auto-tracks the version users have) lands with CHROME.md D8 — the earlier claim that branded Chrome "can't run extension automation at all" was disproven 2026-07-18 (the Puppeteer pipe + CDP-install route works; only `--load-extension` is dead there). Switching the CfT job to the full Chrome E2E parity suite is a deliberate follow-up (runtime cost on CI runners).
...
- **Check both floors:** If you reach for a `browser.*`/`chrome.*` API, verify it exists on both the minimum supported Firefox (152) and Chrome (148) before writing the test — or route the divergence through the `api` capability seam's per-platform wrappers instead of assuming one engine's behavior.

## Project Shape

The WebExtension source lives under `webextension/`. Background scripts run as a non-persistent **event page** (MV3, `background: {"scripts": ["lib/background-main.js"], "type": "module"}` — no service worker). `lib/background-main.js` is a single ES-module entry that named-imports `common.js`/`prefs.js` (real `export`s; zero `globalThis` bridge assignments remain anywhere in `webextension/`) — and then registers every listener directly, reaching the rest of the background (`lib/messages.js`'s dispatch table, `lib/tiles-store.js`, `lib/db.js`, `lib/capture.js`, `lib/backup.js`, `lib/platform.js`) via real `import`s. (The per-slice story of how this settled, including the retired `background.js`/`tiles.js`/`export.js`/`lib/zip-global.js` bridge files and the page-modules arc, lives in git history, `CHANGELOG.md`, and `audit/`.) The new tab page is registered via `chrome_url_overrides.newtab` and lives in `newTab.html` (HTML5, parsed by jsdom's default HTML parser — no more XHTML/XML-namespace gotcha), loaded through a single `page-main.js` module entry fanning out into ~20 feature modules (see `CONTRIBUTING.md` "Architecture").
...
| [`eslint.config.js`](eslint.config.js) | Flat config (ESLint v10+). Top-level `ignores` excludes the vendored zip tree (`webextension/lib/zip/**`, the unbundled ESM `@zip.js/zip.js` core build — see `scripts/update-zip.mjs`). One file-glob block covers all of `webextension/**/*.js` as **module-mode** — page files, `lib/`, and `action.js` are all real ES modules now (the former script-mode block for classic `<script>`-loaded page files retired in the page-modules arc — see git history/`CHANGELOG.md`). `no-unused-vars` set to `caughtErrors: 'none'` so legacy `} catch (ex) {}` blocks don't flag. |
...
| [`tests/e2e/run_esr_tests.sh`](tests/e2e/run_esr_tests.sh) | Lifecycle orchestrator (name unchanged from the ESR era): `pkill` stray processes on this run's profile → `web-ext run --firefox=firefox --pref=extensions.background.idle.timeout=10000 --args="--remote-debugging-port=9222 -headless"` (release channel by default; `$FIREFOX_ESR_BIN` overrides) → wait for port 9222 → `vitest run --project e2e` → cleanup via EXIT trap. Must be executable (`chmod +x`). Also holds a `mkdir`-based concurrency lock (`tests/e2e/.runner-lock`, PID-checked, stale locks reclaimed): a second concurrent invocation refuses to start rather than clobbering the first run's shared profile dir/port. |
...
| **E2E (Chrome parity)** | `tests/e2e/` (same files) + `tests/e2e-chrome/` (runner/helpers) | Vitest + Puppeteer + **branded stable Chrome** (Decision 12; CfT fallback) via CDP, port 9223 | `pnpm test:e2e:chrome` | Alongside the Firefox E2E tier for Chrome-affecting changes; see `tests/e2e-chrome/README.md` |
...
| **UAT (Chrome)** | `tests/uat/` (same scenarios) | Claude Code (headless) + thin MCP client → browser daemon (Selenium + Chrome for Testing), port 9877 | `pnpm test:uat:chrome` | Pre-release only; never on PR/CI |
...
### Browser binary strategy (version fidelity — Decision 12, 2026-07-18)

**Principle: every browser gets two lanes. The frequent tier (E2E) runs the *production* binary users actually have; the pre-release tier (UAT) runs the *current/dev* binary. Binary divergence then surfaces as tier disagreement instead of hiding behind a single binary's quirks.** Full decision record + probe evidence: [`CHROME.md`](CHROME.md) Decisions 11–12 and arc D8.

| Lane | Firefox | Chrome |
|---|---|---|
| **E2E** (frequent — production binary) | **ESR** the day a ≥ 152 ESR ships; release-channel until then | **branded stable** `google-chrome` (pipe-transport launcher; CfT fallback) — landing with D8 |
| **UAT** (pre-release — current binary) | current release | **CfT** (Selenium daemon) |
| Smoke | — | CfT locally (fast); branded in CI (runners preinstall it) |
| Manual pass (D-gates) | release, real profile | branded stable, real profile |

Why this exists — the **canary-gate incident (2026-07-17)**: all Chrome tiers ran on CfT, an unbranded build whose channel detection passes canary-only feature gates. The `message_serialization: "structured_clone"` manifest key was honored by CfT but rejected by branded stable ("requires canary channel"), so production Chrome silently fell back to JSON messaging and every binary wire (thumbnails, favicons, backup, restore) was broken **while every tier stayed green**. Probes confirmed CfT-minus-key ≡ branded stable for messaging semantics — hence the key's removal and the JSON-safe wire codec (CHROME.md Decision 11).

Standing rules that fall out of it:

- **Load-bearing rule:** no capability ships as load-bearing unless proven on the production binary (branded stable Chrome / release-or-ESR Firefox). Official docs + CfT together are insufficient evidence — Decision 10 had both and was still wrong for users.
- **Firefox ESR trigger:** both Firefox lanes can run any channel (temporary-install works everywhere); the only constraint is the 152 API floor. When a ≥ 152 ESR reaches Mozilla's APT repo, flip E2E to ESR (`$FIREFOX_ESR_BIN` / binary default — `run_esr_tests.sh` has carried the name since the ESR era); UAT stays on current release. Until then both run release — safe because `strict_min_version: 152` *excludes* pre-152 users at install time (exclusion, not silent divergence).
- **Chrome lane mechanics:** branded ≥ 137 ignores `--load-extension`; the working automation route is Puppeteer **pipe** transport + CDP `Extensions.loadUnpacked` (+ `--remote-debugging-port` served simultaneously for the test suite — dual-transport probe-proven). Selenium/chromedriver speaks the port transport where the `Extensions` domain is unavailable — which is why the Selenium-based UAT daemon stays on CfT.
- **CfT staleness guard:** the provisioned CfT is cached and drifts against auto-updating stable (in both directions); provision/preflight warns on major-version drift (D8).
...
- **The raw-module-eval tripwire:** [`tests/unit/raw-module-eval.test.ts`](tests/unit/raw-module-eval.test.ts) spawns a raw Node process (no Vitest/vite transform) to import `page-main.js` and asserts the failure is a missing-browser-API `ReferenceError` — never a `SyntaxError` or a TDZ "before initialization" error. Vitest's module transform doesn't preserve TDZ semantics for cyclic imports, so a real cycle-ordering bug can pass the fast tier and only throw in a genuine module loader (the class of bug this test exists to catch). Keep this test green when touching any page-module import cycle.
...
- **Import, not vm-load, for every `webextension/**` module.** All of it — `lib/`, and the page files (`newTab.js`, `grid.js`, `cell.js`, `site.js`, `page.js`, `icons.js`, `awesomebar.js`, `stats.js`, `action.js`, `tiles-shim.js`, `common.js`, `prefs.js`, plus the chrome-prep-era leaf/feature modules — see `CONTRIBUTING.md` "Architecture" for the full list) — is a real ES module with real `export`s, fully typed and split into feature modules (page-modules + chrome-prep C3/C4 arcs — see `CHANGELOG.md`/`audit/`/git history); test with a native `import`, exactly like production code does. `newTab.js` and `grid.js`/`page.js` are the deliberate exception to a *static* import (their cyclic DOM-wiring needs the real markup mounted first): reach them via `tests/integration/_helpers.ts`'s `mountSite`/`ensureSiteEnv()` (a dynamic import) or your own dynamic `import()` following that precedent. The `globalThis` bridge assignments this paragraph used to describe are gone entirely (chrome-prep C3d) — every cross-reference, production and test, goes through a real `import`. Where a test needs to override a real module's shared-singleton state (`Prefs`/`Blocked`/`Filters`/`NeverCapture`/`TileStats`/…), mutate the imported singleton's properties in place per test rather than replacing the binding — a stand-in object assigned to `globalThis` is invisible to code that imports the real binding (or, if you're testing `lib/background-main.js` itself, seed `chrome.storage.local` and let the real `prefs.js` compute it — see `tests/integration/db-wake-race.test.ts`'s header comment for the pattern).
...
- **Chrome parity:** the same suite (same `tests/e2e/*.test.js` files, zero call-site changes) runs against **branded stable Chrome** (Decision 12 — the production binary; CfT fallback when none is present) via `pnpm test:e2e:chrome`, which calls the sibling lifecycle script [`tests/e2e-chrome/run_chrome_tests.sh`](tests/e2e-chrome/run_chrome_tests.sh) (stages the dev build; branded launch = Puppeteer pipe + CDP `Extensions.loadUnpacked` with the suite connecting on CDP port 9223 — landing with CHROME.md D8; the CfT fallback keeps `--load-extension`). Full parity: 124/126 run + 2 SW-lifecycle tests skipped on Chrome (GH #23), 32/32 files, with per-file divergences documented as header notes rather than silent omissions — see [`tests/e2e-chrome/README.md`](tests/e2e-chrome/README.md) and `CHROME.md` D5b. Run it alongside `pnpm test:e2e` (not instead of it) for any change that touches a Chrome-relevant path (the `api` seam, manifest, service-worker-visible code).
- **When to run:**
...
See [`README.md`](README.md) "Scope" for the differentiating-vs-parity framing that drives this E2E-depth split.
...
#### Principled harness driving — and the drag-test quarantine policy

E2E/UAT drive the real UI: no test-only handle on `window`, no production test hooks. Every interaction goes through DOM reads/writes, `runtime.onMessage` wire calls, `browser.storage.local`, or synthesized DOM events — never a page-global read (`window.Grid`, `window.Prefs`, etc.). This was a deliberate maintainer directive: the payoff is that production code carries zero `globalThis` test bridges.

The one accepted cost: `drag-layout.test.ts` and `drag-reorder.test.ts` dispatch real `dragstart`/`drag`/`dragend`/`drop` `DragEvent`s (with a genuine `new DataTransfer()`) on the actual tile/cell DOM nodes, rather than calling an internal `Drag.start()` with a mock event. Synthesized drag gestures in headless Firefox are a known-flaky class. **Quarantine policy:** investigate only after 3 consecutive CI failures of the same drag test; never revert to page-global/internal-API driving as a fix. Both files carry a header comment recording this policy.
...
- **Chrome:** the daemon is parameterized by `$UAT_BROWSER` (`firefox` default | `chrome`) rather than being a second implementation — `pnpm test:uat:chrome` runs the same scenario suite against a Chrome-for-Testing session (Selenium + `--load-extension` of the staged dev build at launch, port 9877 instead of Firefox's 9876), and both daemons can run in parallel. 11/11 scenarios pass on Chrome — see `CHROME.md` D6.
...
- Do not introduce a second test framework or a second mocking library. Chrome support is already in place (`CHROME.md`, [`CONTRIBUTING.md`](CONTRIBUTING.md) "Decisions of record") via the same test frameworks and the `api` capability seam — route new platform divergence through that seam and its E2E/UAT tiers rather than adding an ad-hoc third stack.
