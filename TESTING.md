...
Prefer **behavioral tests** that load the module via a native `import` and exercise it through its public API — every file under `webextension/` (page files and `lib/` alike) is a real ES module with real `export`s since the page-modules arc (see [`PAGE_MODULES.md`](PAGE_MODULES.md)). The two monoliths (`newTab.js`, `fx-newTab.js`) are the one exception: they're reached via a computed-path `await import(/* @vite-ignore */ webextPath('...'))` rather than a static specifier, so `tsc` doesn't follow them into the typed program (deliberate — full JSDoc backfill of ~4.8k monolith lines is a separate arc). The helper in [`tests/integration/_helpers.ts`](tests/integration/_helpers.ts) that reduces the boilerplate for that pair:

- **`mountSite(linkData)`** (async) — constructs a `Site` instance with the full tile environment (mounts the shipped `newTab.html` body, then `ensureSiteEnv()`'s computed-path import of `fx-newTab.js`/`newTab.js`). Returns `{ site, node, cleanup }`; callers must `await` it.
...
- **Import, not vm-load, for every `webextension/**` module.** All of it — `lib/`, and the page files (`newTab.js`, `fx-newTab.js`, `icons.js`, `awesomebar.js`, `stats.js`, `action.js`, `tiles-shim.js`, `common.js`, `prefs.js`) — is a real ES module with real `export`s (page-modules arc, see [`PAGE_MODULES.md`](PAGE_MODULES.md)); test with a native `import`, exactly like production code does. The two monoliths (`newTab.js`, `fx-newTab.js`) are the deliberate exception to a *static* import: reach them via `tests/integration/_helpers.ts`'s `mountSite`/`ensureSiteEnv()` (a computed-path `@vite-ignore` dynamic import), which keeps them out of the typed program. Some page files still keep a `globalThis.X = X` bridge assignment at their file's bottom marked `TEST-ONLY` — that's for E2E/UAT page-context evaluation (`page.evaluate()`), not for fast-tier tests, which should import the real binding instead. Where a test needs to override a real module's shared-singleton state (`Prefs`/`Blocked`/`Filters`/`NeverCapture`/`TileStats`/…), mutate the imported singleton's properties in place per test rather than replacing `globalThis.X` — a stand-in object assigned to `globalThis` is invisible to code that imports the real binding (or, if you're testing `lib/background-main.js` itself, seed `chrome.storage.local` and let the real `prefs.js` compute it — see `tests/integration/db-wake-race.test.ts`'s header comment for the pattern).
