...
| **Chrome for Testing (CfT), >= 148** | auto-provisioned | Chrome **E2E parity** + **UAT** target — only needed if you're running the Chrome tier. Branded Google Chrome (>= 137) cannot run extension automation at all (`--load-extension` is ignored); CfT is provisioned separately via `pnpm chrome:provision`. See [`tests/e2e-chrome/README.md`](tests/e2e-chrome/README.md). | `pnpm chrome:provision` |
...
- **Chrome:** a parallel `chrome-smoke` job provisions Chrome for Testing (`pnpm chrome:provision`) and runs the Chrome smoke checks (`pnpm chrome:smoke` + the Selenium variant) — the runner's preinstalled branded Chrome can't run extension automation at all. Switching that job to the full Chrome E2E parity suite is a deliberate follow-up (runtime cost on CI runners).
...
| **E2E (Chrome parity)** | `tests/e2e/` (same files) + `tests/e2e-chrome/` (runner/helpers) | Vitest + Puppeteer + Chrome for Testing (>= 148) via CDP, port 9223 | `pnpm test:e2e:chrome` | Alongside the Firefox E2E tier for Chrome-affecting changes; see `tests/e2e-chrome/README.md` |
...
- **Chrome parity:** the same suite (same `tests/e2e/*.test.js` files, zero call-site changes) runs against Chrome for Testing via `pnpm test:e2e:chrome`, which calls the sibling lifecycle script [`tests/e2e-chrome/run_chrome_tests.sh`](tests/e2e-chrome/run_chrome_tests.sh) (stages the dev build, launches CfT with `--load-extension` on CDP port 9223). Full parity: 124/126 run + 2 SW-lifecycle tests skipped on Chrome (GH #23), 32/32 files, with per-file divergences documented as header notes rather than silent omissions — see [`tests/e2e-chrome/README.md`](tests/e2e-chrome/README.md) and `CHROME.md` D5b. Run it alongside `pnpm test:e2e` (not instead of it) for any change that touches a Chrome-relevant path (the `api` seam, manifest, service-worker-visible code).
- **When to run:**
