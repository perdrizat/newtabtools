...
- **Status:** built and runnable (`pnpm test:uat`, optionally with scenario slugs to run a subset). Scenarios so far: `01-restore-dogfood` (restore + grid sanity), `02-restore-and-verify` (grid/tile/About assertions), `03-tile-hover-occlusion` (the motivating bug class). Tooling inventory + how to run in [`tests/uat/README.md`](tests/uat/README.md).
