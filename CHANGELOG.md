...
- Every surviving page-file `globalThis.X = X;` bridge assignment is re-marked TEST-ONLY (E2E/UAT page-context evaluation + fast-tier suites that still read a bare identifier off a computed-path dynamic import) — with one production exception: awesomebar.js still reads `Grid`/`newTabTools` as bare globals, so those two bridges stay load-bearing (P2–P5 review finding 1; conversion deferred to keep the monoliths out of the typed program).
- awesomebar.js: dead `typeof Prefs`/`typeof NttIcons` guards dropped (real imports since P4 — permanently true; P2–P5 review finding 2); the `Grid`/`newTabTools` guards stay until the awesomebar conversion.
