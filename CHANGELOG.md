...
- `manifest/chrome.json` opts in to structured-clone messaging (`message_serialization: "structured_clone"`, Chrome 148+; floor raised 144→148 — CHROME.md Decision 10): the thumbnail/favicon wire (`Map`s of `Blob`s) now crosses Chrome messaging intact, closing the tile-rendering gap without touching the Firefox path; smoke 11/11 incl. "tile renders the stored thumbnail".
- CI gains a parallel `chrome-smoke` job (provision CfT + both smokes; the runner's preinstalled branded Chrome can't run extension automation) and package.json gains `test:e2e:chrome` (CHROME.md D5).
- Chrome smoke grows a backup-export check (`Export:backup` → decodable base64 zip payload; 9 checks total, CHROME.md D5) on top of the 8 checks below.
...
- `lib/platform.js`'s `syncActionIconWithTheme` is wired for real (CHROME.md D4, closing the last open box): a new 20th frozen wire message `Theme.colorScheme` (`tests/integration/message-contract.test.ts`) relays the page's `prefers-color-scheme` reading (`theme.js`'s `_initThemeColorSchemeRelay`, sent at boot and on every scheme change) to the background, which calls `api.action.setIcon` with the matching `tools-dark`/`tools-light` PNG pair on a Chrome service worker; no-op on Firefox, where manifest `theme_icons` already handles it declaratively. `scripts/rasterize-icons.mjs` now also rasterizes `tools-dark.svg`.
- UAT on Chrome (CHROME.md D6): `browser-daemon.mjs` is now one `$UAT_BROWSER`-parameterized daemon (`firefox` default | `chrome`) instead of a Firefox-only implementation — Chrome loads the staged dev build via `--load-extension` at launch (no mid-session unpacked-install path exists), port 9877 vs. Firefox's 9876 so both run in parallel.
...
- `lib/messages.js`'s `Export:backup`/`Import:restore` load `lib/backup.js` via a STATIC import again, reversing the 2026-07-09 lazy-import decision: dynamic `import()` is spec-disallowed in service workers (w3c/ServiceWorker#1356) — on Chrome the SW threw and the page saw a null response (first real run of the backup wire, CHROME.md D5); the per-respawn zip-tree parse cost is knowingly re-accepted.
...
- Top-level docs (README, TESTING, CONTRIBUTING) updated to the two-browser reality: Chrome build is E2E/UAT-validated, not dormant, with only store release (D7) pending.
...
- `.ntt-stat-chip` (tile stat badge) was white-on-white in the High-contrast theme — it inherits the theme's foreground text but had no matching background override for `theme="contrast"`; added one using the HC surface/ink tokens (issue #14).
...
- `filters-ui.js`'s `fillFilterUI` no longer makes a 2-argument `api.topSites.get(options, callback)` call unconditionally — Chrome rejects any 2-arg call with "No matching signature", even `get(undefined, callback)`; now branches on `topSitesOptions(api)` resolving `undefined` (the Chrome signal) to the 1-arg `get(callback)` form, keeping the exact Firefox 2-arg call unchanged (CHROME.md D5 pre-work (b)).
