...
- Every surviving page-file `globalThis.X = X;` bridge assignment is re-marked TEST-ONLY (E2E/UAT page-context evaluation + fast-tier suites that still read a bare identifier off a computed-path dynamic import) — no production consumer reads a page global anymore.
